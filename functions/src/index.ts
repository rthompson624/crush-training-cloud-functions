...
exports.processSubscription = functions.firestore.document('subscription/{subscriptionID}').onCreate((doc) => {
  const subscriptionID: string = doc.id;
  const plan: string = doc.get('plan');
  const token: string = doc.get('token');
  const userID: string = doc.get('userID');
  const stripeCustomerID: string = doc.get('stripeCustomerID');
  const stripePlanID: string = doc.get('stripePlanID');
...
exports.updateSubscription = functions.firestore.document('subscription/{subscriptionID}').onUpdate((change) => {
  const stripeSubscriptionID: string = change.after.get('stripeSubscriptionID');
  const stripePlanID: string = change.after.get('stripePlanID');
  const cancelled: boolean = change.after.get('cancelled');
...
exports.recurringSubscriptionPayment = functions.https.onRequest((req, res) => {
  ...
  if (!eventType) throw new Error('No event type designation in request.');
  if (!invoice) throw new Error('No invoice data in request.');
...
exports.createStripeCustomer = functions.firestore.document('user/{userID}').onCreate((doc) => {
  ...
  if (type !== 'trainer') return 'not a trainer';
